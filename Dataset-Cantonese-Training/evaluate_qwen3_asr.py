...
# FIXED VERSION — max_new_tokens moved to from_pretrained()
...
def evaluate_checkpoint(checkpoint_dir: str, test_jsonl: str, output_dir: str,
    ...
    model = Qwen3ASRModel.from_pretrained(
        ...
        max_new_tokens=max_new_tokens,          # ← FIXED: moved here
    )
    ...
    for i, sample in tqdm(enumerate(test_data), total=len(test_data),
        ...
        # Transcribe (NO max_new_tokens here!)
        try:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Qwen3-ASR Evaluation (Fixed)")
    ...
    parser.add_argument("--max_new_tokens", type=int, default=2048,
                        help="Maximum tokens for generation (passed to from_pretrained)")
